...
/*const int minRSSI = -120;  //-120 minimo
const float minSNR = -7;  //5
const int maxTxPower = 14;
const int maxBandwidthIndex = 8;*/
...
void ajustarPotencia(){
  ...
  int ajuste = remoteRSSI > minRSSI ? -1 : (thisNodeConf.txPower < 20 ? 1 : 0);
...
void actualizarPotenciaTx(int cambio){
  ...
  thisNodeConf.txPower = constrain(thisNodeConf.txPower, 0, maxTxPower);
  if(cambio != 0){
...
void ajustarFactorDispersion(){
  ...
  if(cambio != 0){
    ...
    Serial.println(". Nuevo factor: ");
    Serial.print(thisNodeConf.spreadingFactor);
  }
}

int calcularCambioSpreading(){
  if(remoteSNR > minSNR && thisNodeConf.spreadingFactor > 7){
    return -1;
  } else if (remoteSNR < minSNR && thisNodeConf.spreadingFactor < 12){
    return 1;
  }
  return 0;
}
...
void gestionarAnchoBanda(){
  int delta = calcularCambiosBandwidth();/*(remoteRSSI > minRSSI && thisNodeConf.bandwidth_index < 9) ? 1 : (remoteSNR < minSNR < 0 ? -1 : 0);*/
  if(delta != 0){
    thisNodeConf.bandwidth_index += delta;
    thisNodeConf.bandwidth_index = constrain(thisNodeConf.bandwidth_index, 0, MAX_BANDWIDTH);
    Serial.print(delta > 0 ? "Ancho de banda incrementado" : "Ancho de banda reducido");
    Serial.println(". Nuevo indice: ");
    Serial.print(thisNodeConf.bandwidth_index);
  }
}

int calcularCambiosBandwidth(){
  if(remoteSNR < minSNR && thisNodeConf.bandwidth_index > 0){
    return -1;
  } else if (remoteSNR > minSNR && thisNodeConf.bandwidth_index < 9){
    return 1;
  }
  return 0;
...
void loop() 
{
  ...
  if (transmitting && txDoneFlag) {
    ...
    if(waitingAck && (millis() - tx_begin_ms) > TIMEOUT_MS){
      Serial.println("Timeout waiting for ACK. Reverting configuration");
      restaurarConfuguracion();
      waitingAck = false;
    }
  }
...
void sendMessage(uint8_t* payload, uint8_t payloadLength, uint16_t msgCount) 
{
  ...
  if(LoRa.parsePacket() > 0){
    Serial.println("ACK received by Master");
    waitingAck = false;
  }
}
